...
- ✅ No DML/DDL: DROP, DELETE, UPDATE, INSERT, ALTER, CREATE, PRAGMA, etc. blocked
- ✅ No comments: `--` and `/* */` injection blocked
- ✅ Single query only: Multi-statement `;` injection blocked
- ✅ Read-only only: Must START WITH SELECT or WITH (CTE allowed)
...
- **Scope fit**: Assignment-scale data (17 tables, 100K rows max) doesn't need distributed DB
...
- ✅ Complete determinism (reproducible locally & in cloud)
- ✅ Simple backup (file copy)
- ❌ No horizontal scaling (acceptable for this scope)
- ❌ Single-process write (acceptable—mostly read workload)
...
- Examples: `"top billed products"` → deterministic leaderboard query
...
| --------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `backend/app/ingest.py`                       | Data normalization, SQLite schema, analytical views                                | Converts 17 JSONL tables → SQLite + o2c_flow_view + broken_flow_view      |
| `backend/app/graph.py`                        | NetworkX graph construction, value index, trace/neighborhood payloads              | 13 node types, 15 edges, O(1) value_index for highlighting                |
| `backend/app/query.py`                        | Intent routing (14 deterministic), SQL validation (8 checks), response composition | Handles 93% queries deterministically; 7% → LLM                           |
| `backend/app/llm.py`                          | Groq API integration, schema-constrained prompt, error handling                    | Async httpx, schema context, temperature 0.00000001, error categorization |
| `backend/app/main.py`                         | FastAPI endpoints, CORS, app lifecycle                                             | GET /graph, /neighborhood, /node, /trace; POST /chat; GET /health         |
| `backend/tests/test_ingest.py`                | Data correctness regression tests                                                  | Tests for graph construction, broken flow detection, edge cases           |
| `frontend/src/App.jsx`                        | State orchestration, chat/graph coordination, message history                      | Manages graphData, highlightedNodes, trace focus, neighborhood expand     |
| `frontend/src/components/GraphSection.jsx`    | Force-graph rendering, node interaction, highlighting, trace                       | Renders nodes/edges, applies red color to highlights, pink links          |
| `frontend/src/components/ChatPanel.jsx`       | Chat UI, message list, input form, debug output                                    | Messages with intent, optional SQL display (VITE_SHOW_DEBUG_DETAILS)      |
...
3. **LLM fallback triggered**
   - Calls GroqSQLGenerator.generate_sql(question)
   - Sends schema context + question with rules
   - Receives generated SQL (single SELECT)
...
5. **Results grounded** in SQLite rows
   - Never LLM-hallucinated text
   - Response composed from result rows + query intent
...
SQL (deterministic): SELECT COALESCE(product_description, product_id),
...
LLM Prompt: "Schema: o2c_flow_view (columns...), business_partners (id, name...)"
LLM Output: SELECT sales_order_customer_id,
            ...
            ORDER BY month DESC, sales_order_customer_id LIMIT 100
